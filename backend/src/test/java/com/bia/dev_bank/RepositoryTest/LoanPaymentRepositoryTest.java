...
public class LoanPaymentRepositoryTest {
    ...
    @Autowired
    private LoanPaymentsRepository loanPaymentsRepositoryy;

    @Test
    void shouldSaveLoan() {
        Customer customer = new Customer(null, "Ana", "ana@mail.com", "1234","1990-01-01", "11111111111", "11999999999", null);
        customer = customerRepository.save(customer);

        Loan loan = new Loan(null, LoanType.PERSONAL, LocalDate.now(),
                new BigDecimal(10000.0), 10, new BigDecimal(0.0), new BigDecimal(0.0), new BigDecimal(0.1), customer, List.of());
        loan.calculatedLoanDetails();

        Loan saved = loanRepository.save(loan);
        loanRepository.delete(loan);
        var loanPayments = new LoanPayments(null,LocalDate.now(), PayedStatus.TO_PAY,
                new BigDecimal(500),new BigDecimal(500),new BigDecimal(0.2),new BigDecimal(500),null,loan,List.of());
