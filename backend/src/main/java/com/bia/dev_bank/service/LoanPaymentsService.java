...
public class LoanPaymentsService {
    ...
    public LoanPayments getLoanPaymentsById(Long id){
        var loan = loanPaymentsRepository.findById(id).orElseThrow(
    ...
    public TransactionResponse addTransactionToLoanPayment(Long loanPaymentId, TransactionRequest request) {
        var payment = getLoanPaymentsById(loanPaymentId);
        var account = accountRepository.findByAccountNumber(request.destinyAccountnumber()).orElseThrow(()-> new EntityNotFoundException("NOT FOUND"));
    ...
    public void updateLoanPaymentStatus(LoanPayments payment) {
        Double expected = Double.parseDouble(payment.getPaymentAmount() != null ? String.valueOf(payment.getPaymentAmount()) : "0");
        Double paid = Double.parseDouble(payment.getPaidAmount() != null ? String.valueOf(payment.getPaidAmount()) : "0");
